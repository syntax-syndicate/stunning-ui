  ...
  setup(props) {
    ...
    const digitWheel = computed((): object => {
      ...
      return new Array(10).fill(0).map((item, index) => {
        const inRadius =
          state.digitHeight / 2 / getTanFromDegrees(DIGIT_DEGREE / 2)

        return {
          value: index,
          style: {
            transform: `rotateX(${
              0 - index * DIGIT_DEGREE
    ...
    const digitWheelStyle = computed((): object => {
      const transDuration = `${
        props.duration + (props.stagger ? 200 : 0) * props.index
      }ms`
    ...
    const textStyle = computed((): object => {
      const sizePreset = Object.prototype.hasOwnProperty.call(
        fontSizePreset,
        props.size
      )
